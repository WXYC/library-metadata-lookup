...
URLs cannot be discarded (``url`` nulled, ``found`` demoted, or row deleted)
...
_INSERT_TRACK = (
    ...
    "(album_id, artist, title, spotify_url, deezer_url, resolution_status) "
...
async def set_up_catalog_schema(pg_pool, pg_source):
    ...
    stay intact. Guard functions survive the table drops; the bootstrap's
...
class TestSchemaBootstrap:
    ...
    async def test_track_unique_on_album_artist_title(self, pg_pool):
        ...
        async with pg_pool.acquire() as conn:
            await conn.execute(
                _INSERT_TRACK, album_id, "Stereolab", "Pop Quiz", None, None, "pending"
            )
            with pytest.raises(asyncpg.UniqueViolationError):
                await conn.execute(
                    _INSERT_TRACK, album_id, "Stereolab", "Pop Quiz", None, None, "pending"
...
class TestTrackRowGuard:
    ...
    async def test_pending_to_resolved_is_allowed(self, pg_pool):
        ...
        async with pg_pool.acquire() as conn:
            track_id = await conn.fetchval(
                _INSERT_TRACK, album_id, "Juana Molina", "la paradoja", None, None, "pending"
            )
            await conn.execute(
    ...
    async def test_nulling_a_found_track_url_is_rejected(self, pg_pool, url_column):
        album_id = await _make_album(pg_pool)
        async with pg_pool.acquire() as conn:
            track_id = await conn.fetchval(
                _INSERT_TRACK,
