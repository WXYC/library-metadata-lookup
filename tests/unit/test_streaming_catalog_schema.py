...
class TestSetUpStreamingCatalogSchema:
    ...
    async def test_preamble_bounds_lock_waits_and_serializes_boots(self):
        # LML#1038 PR-2: the preamble text is now generated by the shared
        # entity.ddl.bootstrap_lml_cache_table, called with
        # advisory_key=_BOOTSTRAP_ADVISORY_LOCK_KEY -- this table keeps its
        # inner xact lock (unlike entity/streaming_url_cache.py's retired
        # one) because of the non-lifespan offline-DAO caller.
        pg = _FakePgSource()

        await set_up_streaming_catalog_schema(pg)

        assert pg.statements[0] == "SET LOCAL lock_timeout = '10s'"
        assert pg.statements[1] == f"SELECT pg_advisory_xact_lock({_BOOTSTRAP_ADVISORY_LOCK_KEY})"
    ...
    async def test_bootstrap_is_pure_ddl_after_the_preamble(self):
        ...
        assert pg.statements[:2] == [
            "SET LOCAL lock_timeout = '10s'",
            f"SELECT pg_advisory_xact_lock({_BOOTSTRAP_ADVISORY_LOCK_KEY})",
        ]
        ddl = pg.statements[2:]
