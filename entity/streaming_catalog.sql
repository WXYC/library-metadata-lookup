...
--
-- Never apply it statement-by-statement without those flags.
--
...
-- One row per deduplicated library album. GENERATED BY DEFAULT (not ALWAYS)
-- on this identity and streaming_track_result's: the one-time seed inserts
-- the legacy SQLite ids verbatim so track_results.album_id references stay
-- valid, then advances each sequence past max(id). library_ids/formats are
...
-- The discogs_* columns are the Discogs match group — album identity, not a
-- streaming probe result, hence kept here rather than as a service row.
...
CREATE TABLE IF NOT EXISTS lml_cache.streaming_album_service (
    ...
    CONSTRAINT streaming_album_service_valid CHECK (
        service IN ('spotify', 'deezer', 'apple_music', 'bandcamp', 'tidal', 'youtube_music', 'soundcloud')
    ),
...
-- and corrupt the next boot's extraction.

DO $catalog_check$
DECLARE
    existing_def text;
    existing_services text[];
    code_services text[] := ARRAY['spotify', 'deezer', 'apple_music', 'bandcamp', 'tidal', 'youtube_music', 'soundcloud'];
    merged_list text;
BEGIN
...
-- unlinking an album's Discogs match, lowering a coverage baseline, any
-- DELETE, any TRUNCATE) is rejected at the database unless the transaction
-- opts in:
--
--   BEGIN;
...
-- fine — a re-run that found the same coverage).
