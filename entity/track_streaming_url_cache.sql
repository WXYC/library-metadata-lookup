...
-- Statements come verbatim from `entity/track_streaming_url_cache.py`; do not hand-edit
-- this file -- a per-module unit test (`tests/unit/test_track_streaming_url_cache_schema.py`)
-- pins the statement text. The runtime source of truth is
-- `entity/track_streaming_url_cache.py`'s `set_up_track_streaming_url_cache_schema`, which issues these statements
...
BEGIN
    SELECT pg_get_constraintdef(oid) INTO existing_def
        FROM pg_constraint
        WHERE conrelid = 'lml_cache.track_streaming_url_cache'::regclass
            AND conname = 'track_streaming_url_cache_service_valid';
    IF existing_def IS NOT NULL THEN
        ...
        IF inner_array IS NULL THEN
            RAISE WARNING 'track_streaming_url_cache_service_valid: deployed CHECK (%) is not in the '
        ...
        IF rebuilt_array IS DISTINCT FROM inner_array THEN
            RAISE WARNING 'track_streaming_url_cache_service_valid: deployed CHECK (%) has literals this '
    ...
    EXECUTE 'ALTER TABLE lml_cache.track_streaming_url_cache '
        'DROP CONSTRAINT IF EXISTS track_streaming_url_cache_service_valid, '
        'ADD CONSTRAINT track_streaming_url_cache_service_valid CHECK (service IN ('
        || merged_list || '))';
END;
$widen_service_check$;
