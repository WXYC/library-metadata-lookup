...
class Settings(BaseSettings):
    ...
    lml_override_requires_floor: bool = Field(
        ...
        description=(
            ...
            "it is bound. ALL FIGURES BELOW ARE WXYC MEASUREMENTS (2026-09), not "
            "properties of this feature -- they are the WXYC catalog crossed with one "
            "hand-curated Discogs import, and an adopter must re-derive them against "
            "their own corpus before trusting any of them. Of 61,046 pins, 20,520 point "
            "at a release absent from the discogs cache and 892 at an absent card, "
            "leaving 39,634 gradeable; 7,125 of those (18.0%) fail the floor, and "
            "demoting them changes 590 bindings (1.0% of the corpus) -- "
            ...
            "two pressings of the same album and is NOT the lever for wrong-pressing "
            "bindings; see WXYC/library-metadata-lookup#1340. On a floor failure the "
            "pin is DEMOTED, never dropped: a "
            ...
            "pin would regress correct pins on defective card rows -- the matcher "
            "scores against the same broken strings the pin failed, so a drop turns a "
            "correct binding into a no-match. 85.3% of floor failures land in that "
            "row (bounded [51.6%, 85.3%]; the band is the population whose candidate "
            "search falls through to the Discogs API, unmeasured offline), so the "
            "common case of this gate is added cost for an unchanged binding. "
            ...
            "where the prefetch is skipped entirely. Two portability caveats an "
            "adopter needs before flipping this. (1) The floor itself, "
            "SCORE_MATCH_ACCEPTANCE_FLOOR = 80.0, is a bare constant with no config "
            "surface, calibrated against WXYC's corpus; this flag promotes it from a "
            "search-ranking heuristic to an authority that can DISCARD curated data, "
            "which is why it needs a re-derivation procedure -- see "
            "WXYC/library-metadata-lookup#1342. (2) Coverage scales with cache "
            "completeness: a pin whose release has no cache row is exempt by the "
            "degrade contract, which is 33.6% at WXYC and approaches 100% in the "
            "documented degraded mode (library.db + a Discogs token, no discogs-cache "
            "tier), where this flag would be inert. Measure before concluding it works. "
            "Ship dark; flip in staging and watch the binding delta (the "
            "override_pin_* cache-stats counters) before prod. See "
    ...
    lml_bulk_spotify_streaming_warm: bool = Field(
        ...
        description=(
            ...
            "path, and per BS#1747 the enrichment worker stops calling LML for an "
            "album once its artwork_url/discogs_url is non-null, so the yield "
            "accrues to future playcuts of the same normalized "
