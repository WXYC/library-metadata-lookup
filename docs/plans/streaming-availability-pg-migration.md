...
| 9 | ~~`scripts/match_compilations.py`~~ | R | **SUPERSEDED by LML#1020 D6** — its matching cascade moved to `scripts/_lib/release_matching.py` (a shared-script-helper, not a standalone script) and its JSON-writing CLI was retired outright; there is nothing left at this path to port |
...
| 12–18 | `canonicalize_albums.py`, `enrich_discogs_matches.py`, `spotify_artist_catalog.py`, `musicbrainz_matching.py`, ~~`merge_cta.py`~~, `revalidate_spotify.py`, `validate_streaming_urls.py` | mixed | **Leave as SQLite-era artifacts** — all archived one-offs (2026-04-25 archive commit). Add a header note: they run only against a SQLite snapshot produced by the new export tool. Do not port. `merge_cta.py` itself was **deleted outright by LML#1020 D6** (it SSH-wrote to tubafrenzy MySQL, which the cross-cache-identity pivot's no-cross-service-writes rule forbids) — there is nothing left at that path either. |
...
5. **PR E — standalone script ports** (`feat/842-e-script-ports`): `discogs_rematch`, `search_unmatched_compilations`, `revalidate_misses`, ~~`match_compilations`~~, `regenerate_report_stats`; archive-header notes on the 7 one-offs. `match_compilations` drops out of this batch — LML#1020 D6 already moved its cascade to `scripts/_lib/release_matching.py` and retired its CLI, so there is no SQLite-era script left at that path for this PR to port.
