...
> **Amendment (post code-review iter-1 + iter-2, plus LML#572 follow-up).** The shipped implementation diverges from the original plan in three ways:
...
> 2. The mint into ``entity.release_identity.apple_music_album_id`` was **deferred** in PR #571 and re-introduced by LML#572. ``apple_music_album`` is now wired through ``identity.release_validation`` — ``RELEASE_SOURCE_COLUMN["apple_music_album"] = "apple_music_album_id"``, the sentinel rule in ``validate_and_canonicalize_external_id`` accepts positive decimal integer strings (rejects ``"0"`` and non-numeric inputs, same posture as the Discogs branch), and ``coerce_external_id`` carries a TEXT branch that returns the string verbatim (the store wraps it via ``to_pg_text_form`` at the write boundary). The post-process now calls ``mint_or_get_release_identity("apple_music_album", album_id)`` on ``live_resolved`` outcomes only — cache hits already minted, and re-minting would write redundant reconciliation log rows. The mint is best-effort: failures (PG outage, validation rejection, unparseable URL) are logged and swallowed so the URL still surfaces in the response. The public ``POST /api/v1/identity/resolve`` endpoint does NOT accept ``apple_music_album`` yet — that requires the matching enum entry in ``wxyc-shared/api.yaml``'s ``ReleaseIdentitySource``. The post-process mint sidesteps the pydantic gate by calling ``mint_or_get_release_identity`` directly (after going through ``identity.release_validation``); the source is on the ``_INTERNAL_ONLY_SOURCES`` allow-list in ``tests/unit/test_release_validation.py`` until the wxyc-shared follow-up lands.
