...
4. The headline — artist-shelf rows that could gain track-level recall with **no new data collection** vs. rows that would need some, with the latter split by *which remedy moves it*: rows the filter admits nothing for (structural — no candidate exists to score, so nothing inside LML reaches them) vs. rows whose release is admitted but cleared no title floor (a matcher change moves these). The script also reports rows rescued by a *sibling* shelf row's admission, which is the only way to know whether the two legs' asymmetry below actually pays off.
