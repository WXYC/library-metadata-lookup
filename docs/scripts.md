...
**How a rejection is handled ([LML#1353](https://github.com/WXYC/library-metadata-lookup/issues/1353)).** A rejection may be relaxed to the title axis alone, but only for candidates where `va_artist_axis_is_uninformative` holds — both the shelf credit and the candidate credit are V/A credits, so the artist score carries no information. A candidate credited to a *named* artist is refused outright and the row stays a miss. The decision logic and the argument for it live in `scripts/_lib/match_decision.py`'s module docstring; the two lanes differ in how they reach it:
