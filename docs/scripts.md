...
2. Of the artist-shelf rows, how many resolve to a specific Discogs release today, via an exact-case-folded artist-name match cross-checked at the same 80/80 floor `clients/streaming/matching.py::find_best_typed_match` already applies for artwork resolution (LML#478) — reused read-only through its existing `query_artist` variant-iterable support, with no change to the floor.
3. Of those, how many carry a cached tracklist (`release_track` rows). **Non-discriminating against an unfiltered source — never read as coverage; see the caveats below.**
4. The headline — artist-shelf rows that could gain track-level recall with **no new data collection** vs. rows that would need some, with the latter split into its two populations (no Discogs release under any of their artist names / matched an artist but no title cleared the floor), because those want two different remedies.

**The methodology caveats live in the script's module docstring, and the script prints them alongside the numbers they qualify — that is the canonical copy.** In summary: every Discogs-sourced figure is an **upper bound** (the real prod discogs-cache is *filtered* to releases whose credited artist case-folds against a WXYC library artist; a full unscoped dump is not, so the numbers describe what would be resolvable if the cache held everything the dump does); **number 3 cannot discriminate** against an unfiltered source, because practically every release in a full dump carries `release_track` rows; and the constraint that actually binds — membership in the filtered prod discogs-cache — is **not measured**, since prod access is out of the script's scope. The `lml_cache.library_release_override` pin-coverage figure is quoted from the `identity/bulk_resolve.py` code comment rather than derived, and is labeled as such. `TRACKLIST_CHECK_CAVEAT` and `DOCUMENTED_PIN_COVERAGE_NOTE` travel with the figures in both the console report and the `--out` JSON, so a reader who meets a number without this doc still meets its caveat.

Item 2's match mirrors `scripts/build_filtered_discogs.py`'s own cache-build filter, **including that filter's union of `library.artist` with `library.alternate_artist_name`** — narrowing it to the `artist` column alone silently overstates the gap, because the alternate-name rows skew hard toward the compound-credit shape the ticket is about. That fork is deliberate (the real filter returns a flat set for a bulk insert; the census needs per-row variants) and is pinned by `tests/unit/test_measure_track_recall_gap_filter_parity.py` rather than left as a prose claim.

Omit `--discogs-url` (and unset `DATABASE_URL_DISCOGS`) to run the library-only half of the census with the Discogs-side measurement cleanly skipped, rather than pointed at nothing. Such a run reports `discogs_measurement: "skipped"` and `null` headline figures in the `--out` JSON rather than serializing the derived headline off unmeasured zeroes, which would otherwise read as "measured: nothing is resolvable."
