...
- **A release whose only URL was a gated Spotify one is dropped**, not inserted as an all-NULL row, which would otherwise read as "on streaming" to `POST /admin/upload-library-db`'s streaming diff.

The gate goes inert with a `WARNING` on a database whose `albums` table lacks the two provenance columns — absent columns mean "cannot answer", not "no provenance" — which is what keeps discogs-etl's sync e2e fixture green. The column check is casefolded, since SQLite resolves column names case-insensitively while `PRAGMA table_info` reports the declared case.
