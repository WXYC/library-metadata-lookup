...
**How a rejection is handled ([LML#1353](https://github.com/WXYC/library-metadata-lookup/issues/1353)).** Each lane runs the guarded matcher first (`find_best_match`, the shared 80/80 floor). When it rejects everything, the lane may fall back to the title axis alone — but only for candidates where `va_artist_axis_is_uninformative` holds, i.e. both the shelf credit and the candidate credit are V/A credits, so the artist score is carried by a shared "Various Artists" prefix and identifies nothing ([LML#1139](https://github.com/WXYC/library-metadata-lookup/issues/1139), [LML#1147](https://github.com/WXYC/library-metadata-lookup/issues/1147)). A candidate credited to a *named* artist is refused outright: there the artist axis has information and it said no. Until this fix the fallback took any title score ≥ 80 whatever the credit, first in the service's own ordering, which is how album 52656 ("Married to the Mob", the 1988 soundtrack) came to hold Speaker Knockerz's "Married to the Money". Best-of replaced first-of; ties break by ascending URL/release id. The decision logic is `scripts/_lib/match_decision.py`, shared by all three lanes.

**`found_title_only`.** A title-only acceptance is written to `{service}_status` as `found_title_only`, never `found`, so `{service}_confidence` has exactly one meaning per row: the score of the axes the row names. It is also the predicate a serve-side gate can filter on — note that `scripts/export_streaming_links.py`'s album-level export is `spotify_url IS NOT NULL` today, with no status or provenance condition. Every write (both lanes, both statuses) now sets `{service}_matched_artist`, `{service}_matched_title` and `{service}_checked_at` in the **same** `UPDATE` as the URL; the artifact already holds rows whose provenance is stale relative to their URL (album 29493 stores `matched_title` "S.F. Sorrow" at confidence 100 beside a URL resolving to "Silk Torpedo"), which is what splitting the two writes across passes produces. A title-only decision will not overwrite a row already at `found` — a guarded match outranks a one-axis one, and the mirrored PG table raises on that demotion.

The Discogs lane keeps its own historical 70 title floor and is not status-flagged: `albums` has no `discogs_confidence` column, so nothing there persists a score, and `discogs_status` is policed by a found-demotion trigger on the PG side. `scripts/discogs_rematch.py` is the same shape at one remove — its passes score one axis each (pass 1 artist, pass 2 title) and report which in an `axes` field, but neither is a title-only acceptance and neither persists the number.
