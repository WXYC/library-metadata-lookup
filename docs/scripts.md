...
Four numbers, in order: (1) comp-shelf vs artist-shelf split of `library.db`, via the same `wxyc_etl.text.is_compilation_artist` classifier `scripts/build_compilation_track_location.py` gates on — not a hand-rolled `LIKE 'Various Artists%'` guess, which the script also reports side by side (`comp_shelf_naive_like_count`) because the two counts diverge (the classifier catches shelf forms like bare `"V/A"` and lowercase `"various"` the naive heuristic misses); (2) of the artist-shelf rows, how many resolve to a specific Discogs release today, via an exact-case-folded artist-name match (mirroring `scripts/build_filtered_discogs.py`'s own cache-build filter, **including its union of `library.artist` with `library.alternate_artist_name`** — see `artist_variants`, and note that narrowing this to the `artist` column alone silently overstates the gap, because the alternate-name rows skew hard toward the compound-credit shape the ticket is about) cross-checked at the same 80/80 floor `clients/streaming/matching.py::find_best_typed_match` already applies for artwork resolution (LML#478) — reused read-only via its existing `query_artist` variant-iterable support, not reimplemented and with no change to the floor; (3) of those, how many carry a cached tracklist (`release_track` rows) — **non-discriminating against an unfiltered source; see below**; (4) the headline — artist-shelf rows that could gain track-level recall with **no new data collection** vs. rows that would need some.
...
**Number 3 is non-discriminating against an unfiltered source, and must not be read as coverage.** Practically every release in a full Discogs dump carries `release_track` rows (19,341,286 of 19,341,287 in the 2026-08 local `discogs_full`), so item 3 reports ~100% of item 2 whatever the real state of track data. That is not a circular join — the item-2 candidate SQL never touches `release_track` — the check is simply vacuous at this source, and the proposition "tracklist availability is solved" was never tested. Read it as "no upstream track-data gap blocks these rows," nothing more. The constraint that actually binds is whether a release is present in the **filtered prod discogs-cache at all**, which this script deliberately does not measure (prod access is out of scope). `TRACKLIST_CHECK_CAVEAT` travels with the figure in both the console report and the `--out` JSON so a reader who meets the number without this doc still meets the caveat.
