...
### The #428 streaming-URL pin

[WXYC/wxyc-shared#428](https://github.com/WXYC/wxyc-shared/issues/428) annotated the five streaming URL fields (`spotify_url`, `apple_music_url`, `youtube_music_url`, `bandcamp_url`, `soundcloud_url`) with `format: uri`, which `datamodel-code-generator` maps to pydantic `AnyUrl` — a **decode-time** validator. Unpinned, a regen against api.yaml >= 1.50.2 would turn every malformed `streaming_links` row already persisted in this service into a hard read failure, so the script holds those five at `str` ([#1299](https://github.com/WXYC/library-metadata-lookup/issues/1299)). The pin is a name-keyed substitution plus an AST post-condition that fails the run with `#428 pin did not apply` if the substitution silently no-ops — which it does whenever the generator emits a shape the line anchor doesn't match, e.g. `Annotated[AnyUrl | None, Field(...)]` under `--use-annotated`. Both failure modes are driven end to end in `tests/unit/test_generate_api_models_pin.py`. Scope is deliberately those five names: the archive presigned-GET `url` and the OAuth device-flow `verification_uri`/`verification_uri_complete` are `format: uri` too and keep `AnyUrl`, which a codegen-wide type override could not express (`--field-constraints` and `--strict-types` are file-wide once any override targets `format: uri`). The mechanism is ported from wxyc-shared's canonical `scripts/generate-python-models.sh`; that script's header holds the canonical rationale — read it before changing either copy.
...
`ytmusicapi` became a **runtime** dependency in LML#1103, when the YouTube Music client also became reachable from `/lookup` (the streaming-URL cache warm). The `drain` extra is kept as a stable name — the `--extra drain` invocations below still work — but it is now empty, and a plain `uv run` would serve this drain equally well. The client's import stays lazy, so a process that never resolves YouTube Music still never pays it. `--execute` is opt-in; the default is dry-run (writes nothing). Persisting is the operator's action (off-peak, fill-only, one bulk consumer at a time), followed by a `POST /admin/upload-streaming-db` republish.
...
## Unmatched-Compilation Drain (`scripts/search_unmatched_compilations.py`)

Searches `is_compilation = 1` rows that no other pass has resolved, first against the Discogs PG cache by title (Phase 1), then Deezer + Spotify album search for the Discogs misses (Phase 2). WXYC files compilations under a shelf convention rather than an artist name ("Various Artists - Blues", "Soundtracks - M"), so `build_compilation_query` strips the filing prefix and the query often has no artist at all.

**How a rejection is handled ([LML#1353](https://github.com/WXYC/library-metadata-lookup/issues/1353)).** All three lanes pick their rule from the **query credit**, and the two branches are disjoint by construction, because `build_compilation_query` does not always reduce a compilation's `display_artist` to a V/A one — its "X mixes various artists" arm and its "real artist misclassified as a compilation" fall-through return real names:

| query credit | rule |
|---|---|
| **A V/A credit** | the title axis alone, and only against V/A-credited candidates (`va_artist_axis_is_uninformative`). The artist axis is deliberately *not* scored: the query side is the `Various` sentinel the drain substitutes, not catalog data, so a candidate credited exactly "Various" scores 100 against it for free — a tautology, not a marginal [LML#1139](https://github.com/WXYC/library-metadata-lookup/issues/1139) prefix clear — and would otherwise be recorded as a two-axis `found`. That is the population most likely to be a wrong V/A link, so it is the one that most needs the one-axis marker |
| **A recovered real name** | the guarded 80/80 matcher. The relaxation cannot serve it — it needs a V/A credit on both sides, so it refuses every candidate — and the artist axis genuinely carries information here |
...
**What the status does and does not buy, today.** It is not a gate anywhere yet. `scripts/export_streaming_links.py`'s album-level export is `WHERE spotify_url IS NOT NULL` plus the LML#1352 match-provenance condition — and a `found_title_only` row **passes** that condition precisely because this change records what it compared, so it still reaches `library.db.streaming_links` and the DJ — now carrying `matched_artist = "Various Artists"` and a `matched_title` identical to the query, which *reads* like corroboration and (see the module docstring) is not. **The entire behavioural protection in this change is the V/A narrowing — refusing to write a named-artist candidate at all.** The status buys legibility, and a predicate a serve-side gate can adopt; it does not itself withhold anything from anyone. [LML#1355](https://github.com/WXYC/library-metadata-lookup/pull/1355) did **not** adopt it and is orthogonal to it by construction: its predicate asks whether provenance is *absent*, which is a different question from whether the provenance present covers one axis or two, and hard-coding this status vocabulary into the export would have coupled the two merge orders. So a one-axis row that records its axes remains exported and remains labelled `verified` downstream; withholding it is still unclaimed, and belongs to the serve seam with the `verified` demotion.
...
Both axes drive the real `find_similar_artist` over a real SQLite catalog, so nothing here forks a production predicate and there is no copy to drift. That is a better answer to the parity requirement than a parity test. A pool that fails to build aborts the run with a `RuntimeError` rather than measuring: an unusable pool suppresses correction entirely, so every row would read 0.0 false corrections at 0.0% recall — perfect-looking garbage — while the script's `logging.ERROR` level hides the only warning that says why.
...
Copies `streaming_availability.db`'s per-service album URLs into `library.db`'s `streaming_links` table, which `/lookup` then serves. Three callers, two of them outside this file's obvious blast radius:

- `discogs-etl/scripts/sync-library.sh`, in the daily library sync (and `discogs-etl/tests/e2e/test_sync_library_e2e.py`, which dynamically loads its `main`).
- this repo's `.github/workflows/refresh-streaming.yml` "Verify export" step, which runs `--dry-run` between the pipeline writing the artifact and `POST /admin/upload-streaming-db` pushing it back to the canonical bucket.

It only ever **reads** the streaming artifact (the single bucket-canonical copy of rate-limited Apple/Spotify/Deezer results), enforced by opening it `mode=ro` rather than by containing no `UPDATE` — which matters most at that second caller, where a read-write open could have hot-journal-repaired the artifact and then uploaded the altered file. `tests/unit/test_export_streaming_links.py` asserts the open mode, a sha256 across a run, and that the mode survives a path containing URI punctuation. The trade is stated at the `connect` call: a stray journal beside the artifact now fails the run instead of being silently rolled back.
...
**The measured census, the two approaches deliberately not taken, and why the gate is this weak live in `_has_match_provenance`'s docstring in that script — that is the canonical copy**, next to the predicate it justifies, rather than restated here where it would drift. In summary: measured on the 2026-09-25 production artifact, 0.83% of the 46,907 `albums` rows with a `spotify_url` (388 rows) are gated; 363 of them are every compilation row in the artifact with a Spotify URL, written by the title-only override in `scripts/search_unmatched_compilations.py` (LML#1353). It is deliberately **not** an identity cross-check against `entity.release_identity` and deliberately **not** widened to the 39% of rows holding a writer tag — both for reasons of measured evidence, not taste, and both argued at the docstring.

Two things to know before sizing a change here:

- **The −388 figure is a count of gated album ROWS, not a `streaming_links` delta.** It is what the census shape yields — one `library_id` per album, no `track_results` — and `streaming_links` is keyed by `library_id`, so the two units differ in both directions: several album rows can share a `library_id`, a row can carry an empty `library_ids`, and the track supplement below can refill a slot the gate emptied. Size a change here off the per-service coverage the script now logs on **both** paths (`--dry-run` and the write path), not off this number. What is safe to say without re-measuring: neither upload guard is engaged, because `POST /admin/upload-streaming-db`'s 5% coverage guard measures the *artifact*, which this script never writes, and `sync-library.sh`'s `STREAMING_APPLE_FLOOR` counts `apple_music_url`, which this gate does not touch. Note the corollary — nothing anywhere guards `streaming_links.spotify_url` itself, which is why the coverage log exists.
- **A release whose only URL was a gated Spotify one is dropped**, not inserted as an all-NULL row, which would otherwise read as "on streaming" to `POST /admin/upload-library-db`'s streaming diff. Dropping is itself a signal in the other direction, though: that diff feeds the LML#1313 streaming webhook, so each dropped `library_id` sends Backend-Service an `on_streaming: false` on the first sync after merge. Disclosed and routed at the drop in the script, not handled here.

The gate goes inert with a `WARNING` on a database whose `albums` table lacks the two provenance columns — absent columns mean "cannot answer", not "no provenance" — which is what keeps discogs-etl's sync e2e fixture green. The column check folds case, since SQLite resolves column names case-insensitively while `PRAGMA table_info` reports the declared case — ASCII-only, matching SQLite's own rule rather than Python's `casefold()`, which over-matches; see `_ascii_fold`.

### Known, unchanged: track-level URLs in an album field

The script supplements album URLs from `track_results`, writing a track-shaped `tr.spotify_url` into the **album** field `entry["spotify_url"]` — the origin of the `/track/`-shaped values Backend-Service serves ([WXYC/Backend-Service#2689](https://github.com/WXYC/Backend-Service/issues/2689)). **The measured counts and the reason this is left alone live in the code comment at that block**, next to the loop they describe; in short, a non-trivial share of artifact albums get their `spotify_url` only from this supplement, so removing or reshaping it is a coverage decision rather than a bug fix. It runs *after* the gate, so a gated album with a resolved track is refilled from it; `tests/unit/test_export_streaming_links.py` pins that interaction so it cannot drift silently while the decision is routed.
