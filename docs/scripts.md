...
- **Coverage delta is an upper bound, not the delta.** `streaming_links.spotify_url` goes 46,907 → 46,519 (−388, −0.827%) when replayed at the census shape — one `library_id` per album and **no `track_results`**. The track supplement below refills a gated slot, so the real loss is smaller and the real *fix* rate is correspondingly lower. Neither upload guard is engaged either way: `POST /admin/upload-streaming-db`'s 5% coverage guard measures the *artifact*, which this script never writes, and `sync-library.sh`'s `STREAMING_APPLE_FLOOR` counts `apple_music_url`, which this gate does not touch.
...
The script supplements album URLs from `track_results`, writing a track-shaped `tr.spotify_url` into the **album** field `entry["spotify_url"]`. That is the origin of the 841 `/track/`-shaped values Backend-Service serves ([WXYC/Backend-Service#2689](https://github.com/WXYC/Backend-Service/issues/2689)). **3,638** artifact albums get their `spotify_url` only from this supplement, so removing or reshaping it is a coverage decision rather than a bug fix and is not made here. It runs *after* the gate, so a gated album with a resolved track is refilled from it; `tests/unit/test_export_streaming_links.py` pins that interaction so it cannot drift silently while the decision is routed.
