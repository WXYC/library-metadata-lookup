...
- `LML_ARTIST_IDENTITY_SPLIT_GATE` -- When `true` (default), the `extended=true` lookup path gates `artist_bio` / `wikipedia_url` / `profile_tokens` / `discogs_artist_id` on the LML#504 `artist_identity_verified` composite predicate: request artist ↔ (`item.artist` OR `item.alternate_artist_name`) ↔ `top1_release.artist` each at `score_match >= 80` (Discogs disambiguation suffix `(N)` stripped before scoring so common-name artists like `Sessa (2)` pass), neither side a `is_compilation_artist` alias, and the Apple Music probe artwork (`artwork_url`) gated on the same library-row hop so a fuzzy-collision row can't surface a stranger's cover. Set the env var to any common false spelling -- `false`, `0`, `no`, `off`, `disabled` (case-insensitive, whitespace-trimmed) -- to revert those fields to the broader `is_album_derived_eligible` gate that PR #500 introduced. Read at request time via `os.getenv` so the knob flips without a redeploy. The split predicate only takes effect when `LookupRequest.extended=true` AND a non-empty `artist` was supplied (album-only lookups fall back to the legacy gate so they don't lose bio); BS forces `extended=true` on every wire call, so the split exercises on all BS write-path traffic immediately. Divergence between the new and legacy gates emits shadow-mode telemetry under category `artist_identity_split_gate` regardless of the flag state, so the rollback signal stays visible while debugging. See LML#504.
