...
- `LML_PERSIST_APPLE_MUSIC_URL` -- When `true`, `/api/v1/lookup` runs a post-process step that resolves Apple Music URLs for every item whose `apple_music_url` came out null, using the **request's** `(artist, album, song)` independent of the item's library row. Successful resolutions are persisted to `entity.album_apple_music_lookup_cache` (keyed by `(artist_normalized, album_normalized)`) so subsequent lookups for the same album hit the cache rather than re-querying Apple Music. Known misses are recorded with a 7-day TTL so unknown albums don't repeatedly hammer Apple. The schema is bootstrapped at startup via `set_up_apple_music_cache_schema` in `main.py` lifespan; if the discogs-cache PG is unreachable, the cache layer degrades to a no-op (every request pays one extra Apple API call). Default `false` so the rollout is reversible without a re-deploy; flip `true` in Railway after one staging tick confirms the post-process is healthy. To roll back: flip `false` (no re-deploy required). See WXYC/library-metadata-lookup#570.
