...
- **Step 3a — Library-Miss Discogs Probe** (`_step_library_miss_probe`, LML#583): When the pipeline missed on an artist+album request, probe Discogs directly (`lookup/strategies/library_miss.py`); a confident match is synthesized as a row-less `LibraryItem(id=0)` pair, bypassing steps 3b and 4. Skipped on `/lookup/bulk` (LML#671). Two LML#784 recall behaviors: a query-side self-titled placeholder ("S/T") swaps to the artist name before searching, and when a cache-served candidate set all floor-fails, the search re-issues once with `skip_pg=True` — the fallthrough seam treats any non-empty `pg_read` as terminal, so without the retry a PG candidate set that scores under the floor would mask an API arm that resolves the pair.
