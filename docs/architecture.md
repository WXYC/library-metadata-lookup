...
- **HALF_OPEN** → one trial in flight (concurrent callers still shed); a genuine 2xx with a healthy remaining CLOSES the breaker, a 429 or still-exhausted remaining re-OPENS it, a 5xx is neutral (stays half-open, next trial decides — a Discogs upstream error is not a rate-limit signal).
