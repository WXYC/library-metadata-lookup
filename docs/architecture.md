...
Every shed increments `discogs_breaker_open_shed` on the per-request `cache_stats` dict — the same #683 `cache.*` counter surface (`cache.discogs_breaker_open_shed` in PostHog, `lml.cache.discogs_breaker_open_shed` on the Sentry transaction, seeded to 0 in `_LML_CACHE_STATS_EXTRA_KEYS`) the row-less flag degradation alerts use, so breaker-open time is alertable. The OPEN/CLOSED transitions are logged once each; per-shed logging is `debug`. Env-var reference in [`env-vars.md`](env-vars.md).
