...
`SWAPPED_INTERPRETATION` is **Discogs-touching** (LML#622): once it identifies the artist side, it cross-references the *other* part as a track and narrows to the release(s) that actually contain it, rather than returning the artist's whole discography. The release→library matching reuses the same kernel as `SONG_AS_TRACK` (`_match_track_releases_to_library` — Discogs track search → compilation-aware `search_album_fuzzy` → deferred tracklist validation, bounded by `_chunked_gather` + a `MAX_SEARCH_RESULTS` early-exit), scoped to the identified artist. When narrowing fires it emits a `matched_via` map (an `Outcome.track_match`, like `SONG_AS_TRACK`); when nothing cross-references — or no `discogs_service` is wired — it returns the artist-filtered result via `Outcome.found` as before. Of the strategies, only `ARTIST_PLUS_ALBUM` is library-only.
