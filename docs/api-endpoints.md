...
`POST /api/v1/artists/resolve/bulk` resolves clean bare artist names — typically touring artists WXYC has never cataloged — to Discogs artist identities, minting `entity.identity` rows for unambiguous resolutions (LML#759). Backend-Service's concerts pipeline (BS#1614) is the consumer. Bearer `LML_API_KEY` auth (401 missing header, 403 wrong/malformed token); max 25 names per request (over-cap returns 413; a fully-escalating batch ≈ 25 Discogs API calls ≈ 30s at the shared 50/min budget, so callers page).
...
The API-verification leg runs serially (the shared limiter paces globally at 50/min; intra-request concurrency would only starve interleaved live-lookup traffic — the LML#370/#372 lesson). Tier orchestration lives in `artists/resolver.py`, the route in `artists/router.py`, the candidate-set queries in `discogs/cache_service.py`, and the `type=artist` search in `discogs/service.py:search_artists`. Observability: an explicit `http.server` Sentry span with `lml.artist_resolve.*` counters (status pinned on every resolver-phase exit — 200/422/499/503/500; requests rejected before the resolver runs produce no span) and one aggregate `artist_resolve_completed` PostHog event per successfully completed request (no per-name events, nothing on error exits — the span carries error-class observability; the resolver's probe counter rides as `discogs_api_calls`, since bare `api_calls` is the framework's per-service map on every `*_completed` event).
