...
- **No `run:` body in these jobs interpolates a `${{ }}` expression.** `${{ }}` is textual substitution *before* the shell parses the script; step 4 passes both SHAs through `env:` (`EXPECTED_SHA`, `PRE_UPLOAD_SHA`) so the shell reads them as data. `github.sha` is safe on its own but follows the same rule, because an all-or-nothing property is the only kind a reviewer can check at a glance.
