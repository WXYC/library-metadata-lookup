...
- **Workflow-level `permissions:`** scoped to the minimum each workflow needs:
  - `ci.yml`, `cross-cache-identity-flags.yml`, `set-railway-var.yml`: `contents: read` (no GITHUB_TOKEN writes). `ci.yml`'s `codegen-check` job additionally exports `github.token` to `scripts/generate_api_models.sh` so its api.yaml download authenticates instead of riding the runner pool's shared anonymous rate budget ([#1205](https://github.com/WXYC/library-metadata-lookup/issues/1205)) — a read within the same `contents: read` scope (wxyc-shared is public), and the script unsets the token before the codegen toolchain runs.
