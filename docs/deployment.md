...
- **`actions/add-to-project@v1.0.2`** in `cross-repo-critical-path.yml`. A third-party action pinned to a release tag (not a SHA), same philosophy as the pins above. Failure mode is quiet by design when `WXYC_PROJECTS_TOKEN` (the org secret this workflow needs) is unset — the step's `if:` skips it rather than failing — but loud once that secret exists: a bad pin then fails the `add-to-project` step outright when a `cross-repo:active` label event fires. Upstream cut a `v2.0.0` (Node 24 runtime bump, no input changes) after this pin was set; not adopted here because `v1.0.2` still runs on the currently-supported `node20` runtime. Revisit if GitHub deprecates that runtime tier, or just to stay current — check https://github.com/actions/add-to-project/releases.
