...
1. **`Sample /health before upload`** runs [`scripts/sample_health_commit_sha.sh`](../scripts/sample_health_commit_sha.sh) *before* `railway up`, reading `/health`'s current `commit_sha` as a step output. It never fails the step — an unreadable or malformed `/health` prints the sentinel `unreachable` and still exits 0, because "no baseline available" is itself meaningful input to the next step, not an error.

   **Both this script and the poller in step 4 read a 503 body rather than discarding it**, and neither uses `curl -f`. `/health` returns 503 with a *fully-populated* body whenever a core dependency is down (`routers/health.py`: `status_code = 200 if status in ("healthy", "degraded") else 503`, with `CORE_SERVICES = {"database"}`), and the `commit_sha` in that body is exactly as accurate as the one in a 200 — what is being read is the deploy's *identity*, not its health. A `curl -f` here would make the whole reconciliation path inert in the situation it exists for: the same Railway incident that times out `railway up` is one that plausibly leaves the live service degraded, and an `unreachable` baseline sends step 4 straight down its unconditional fail-closed branch. Only 200 and 503 are accepted — a 502/504 from an edge proxy has no application response behind it and therefore no deploy identity to read. Health itself remains the smoke-test job's call, downstream.
...
**Why the baseline is treated as untrusted input.** The step-1 value is whatever `jq` pulled out of a remote HTTP response body, and this whole path only runs when the deploy infrastructure is already misbehaving — the moment that endpoint's output is least trustworthy, in jobs that hold `RAILWAY_TOKEN_*`. Two places handle it accordingly, and both are pinned by tests:

- **One reader, used twice.** The poller in step 4 does not re-implement the read — it shells out to the very same `sample_health_commit_sha.sh` the baseline came from, converting that script's `unreachable` sentinel into a retryable non-zero. The status taxonomy, the `curl -f` decision and the null/whitespace rejection are a contract *between* the baseline and the polls, so a second copy would be a contract with itself; an earlier revision kept one and it had already drifted.
- **`sample_health_commit_sha.sh` guarantees exactly one line of stdout.** CI writes it into `$GITHUB_OUTPUT` as `commit_sha=<value>`, a line-oriented format, so a `commit_sha` carrying an embedded newline would break the step or append a second, response-chosen `name=value` line to the step's outputs. A value containing any whitespace is rejected to the same `unreachable` sentinel as every other unusable reading.
- **No `run:` body in these jobs interpolates a `${{ }}` expression** — every step, not a chosen few. `${{ }}` is textual substitution *before* the shell parses the script; step 4 passes both SHAs through `env:` (`EXPECTED_SHA`, `PRE_UPLOAD_SHA`) so the shell reads them as data. `github.sha` is safe on its own but follows the same rule (including in `Record deployed commit SHA for /health`), because an all-or-nothing property is the only kind a reviewer can check at a glance — and the test sweeps every step in the job, so a step added tomorrow is covered too.
...
| `unreachable`, or empty (step 1 could not read a `commit_sha`) | **Fail — inconclusive.** No starting point, no provable transition. |
...
Tunables, analogous to the wait script's: `RECONCILE_HEALTH_TIMEOUT_SECONDS` (900 — deliberately the same budget as the wait script's `RAILWAY_DEPLOY_TIMEOUT_SECONDS`, since both are waiting on the same revision going live, and this one starts its clock earlier: from a failed upload, with the build not yet queued), `RECONCILE_HEALTH_POLL_INTERVAL_SECONDS` (30 — deliberately slower than the wait script's 10, because every poll here lands on our own `/health`, whose `discogs_api` probe makes a live Discogs `GET /oauth/identity` per request outside LML's rate gate; at 10s a full window would spend ~90 calls of a 60/min budget that staging and production share, during an incident), `RECONCILE_HEALTH_CURL_TIMEOUT_SECONDS` (10 — per-poll `curl --max-time`), plus `sample_health_commit_sha.sh`'s own `HEALTH_SAMPLE_CURL_TIMEOUT_SECONDS` (10). A single dropped poll during the reconciliation window is tolerated (mirrors the wait script's transient-error handling) — only never observing the expected SHA before the deadline is a failure.

**Why `/health` is not a weaker gate than the deployment API.** Railway only switches traffic to a revision once `healthcheckPath` passes, and `railway.toml` sets that to `/health`. So observing `github.sha` at the public `/health` *implies* the revision passed the same check `wait_for_railway_deployment.sh` waits for — the two paths converge on one underlying gate rather than this one substituting something softer. It is also why accepting a 503 is sound: a public 503 carrying the *new* SHA is a revision that passed its healthcheck and degraded afterwards, which is still a landed deploy. That chain depends on `railway.toml` keeping `healthcheckPath = "/health"`, so a test pins it.
...
Five classes of pin in `.github/workflows/*.yml` exist for supply-chain reasons (mirrors WXYC/request-o-matic#124's free-tier hardening; see WXYC/wiki#67 for the org-wide rollout). They will bit-rot and need occasional bumps:
...
- **Workflow-level `permissions:`** scoped to the minimum each workflow needs:
  - `ci.yml`, `cross-cache-identity-flags.yml`, `set-railway-var.yml`, `actionlint.yml`: `contents: read` (no GITHUB_TOKEN writes). Two jobs additionally export `github.token` so their raw.githubusercontent.com downloads authenticate instead of riding the runner pool's shared anonymous rate budget — `ci.yml`'s `codegen-check` to `scripts/generate_api_models.sh` for api.yaml ([#1205](https://github.com/WXYC/library-metadata-lookup/issues/1205)), and `actionlint.yml`'s `Download pinned actionlint` to `scripts/install_actionlint.sh` for the actionlint installer ([#1214](https://github.com/WXYC/library-metadata-lookup/issues/1214)). Both are reads within the same `contents: read` scope (wxyc-shared and rhysd/actionlint are public), and both scripts unset the token before running anything else — which for the actionlint one means before executing a third-party script fetched moments earlier.
...
- **`ACTIONLINT_VERSION`** in `actionlint.yml` (the `Workflow Lint` workflow). rhysd/actionlint publishes no root `action.yml`, so the job downloads a pinned binary via the official `download-actionlint.bash` script — the env var feeds both the script's release-tag ref and the binary version it fetches, keeping them in lockstep. The fetch itself lives in `scripts/install_actionlint.sh` rather than inline in the workflow, so its token-safety and retry properties are pinned by `tests/unit/test_install_actionlint_auth.py` instead of reviewed by eye ([#1214](https://github.com/WXYC/library-metadata-lookup/issues/1214)); the wrapper refuses to run without an exact `major.minor.patch` version. Not because an omitted one unpins CI — rhysd's installer resolves nothing over the network; its v1.7.12 copy hardcodes a manually-maintained `version="1.7.11"` default and treats `latest` as a synonym for it — but because the failure is quieter than an unpin: the workflow would keep declaring one tag while a differently-pinned binary got installed. Failure mode is loud (the lint step fails). Bump by checking the latest release at https://github.com/rhysd/actionlint/releases and updating the single `ACTIONLINT_VERSION` value. Last pin: 1.7.12.
- **`actions/add-to-project@v2.0.0`** in `cross-repo-critical-path.yml`. Pinned to an exact release tag rather than a moving major (`actions/checkout@v6`, `actions/setup-python@v6` and `codecov/codecov-action@v5` all self-update within their major; this one does not), which is what puts it on this list. Failure mode is quiet by design while `WXYC_PROJECTS_TOKEN` (the org secret this workflow needs) is unset — the step's `if:` skips it rather than failing — but loud once that secret exists: a bad pin then fails the `add-to-project` step outright when a `cross-repo:active` label event fires. **`v2.0.0` is the floor, not merely the latest.** The prior release `v1.0.2` declares `runs.using: node20`; GitHub deprecated the Node 20 action runtime on 2025-09-19, Node 24 became the runner default on 2026-06-16 (node20 actions now emit a deprecation annotation and are silently force-run on Node 24), and the Node 20 binary is removed from runners on 2026-09-16. `v2.0.0` differs from `v1.0.2` only in that runtime declaration and bundled dependencies — inputs and outputs are identical, so the bump is a drop-in. Do not pin back below it. Bump forward by checking https://github.com/actions/add-to-project/releases.

The `Workflow Lint` workflow (job `actionlint` in `actionlint.yml`) runs `actionlint` on every PR (and `main`/`prod` push) that touches `.github/workflows/**` or `scripts/install_actionlint.sh`, failing on lint errors. It validates `permissions:` syntax, `uses:` reference and `${{ }}` expression/context syntax, and shell-script blocks in `run:` steps (via shellcheck), catching the silent-mistake class of errors above in CI. It is advisory, not a required status check — if you ever mark it required in branch protection, pair it with an always-runs companion job, because a `paths`-filtered check that gets skipped never reports its status and would block every PR that doesn't touch a workflow file. Run `actionlint .github/workflows/*.yml` locally before pushing to get the same signal without burning a CI cycle.
...
3. Re-check `DISCOGS_MAX_CONCURRENT` and `DISCOGS_BREAKER_REMAINING_FLOOR` against the stock N=1 defaults (5 and 3 respectively, `config/settings.py`) — at N=3 these are sized down/up by the same per-worker-division arithmetic the replica knob table uses (staging ran `DISCOGS_MAX_CONCURRENT=1` and `DISCOGS_BREAKER_REMAINING_FLOOR=6` under N=3 — both derived via that table's formulas, not stock values — and both were unset on 2026-09-26 when it went to one worker, so it now takes the stock 5 and 3 like production does; see the note at step 6 of the procedure below).
...
3. **Baseline at `UVICORN_WORKERS=3`** — which staging no longer runs, so this baseline now requires deliberately opening a fresh N=3 window first (see the note at step 6): `LML_API_KEY=... python scripts/gate0_burst.py --host https://library-metadata-lookup-staging.up.railway.app --concurrency 3 --total 12 --warm --json > gate0-n3.json`. Watch stderr for an abort; if it aborts, stop and report rather than retrying immediately (staging's Discogs budget needs to recover first).
...
6. **Restore staging to its prior state** (the values recorded in step 1) once the comparison is recorded, so staging doesn't drift from prod's config between Gate 0 runs.
   ...
   > **Superseded as of 2026-09-26 — do not restore staging to `UVICORN_WORKERS=3`.** Staging is now permanently `UVICORN_WORKERS=1`, for a reason unrelated to Gate 0: [LML#1359](https://github.com/WXYC/library-metadata-lookup/issues/1359) took it to one worker so it matches production's single-heap shape for the [LML#1354](https://github.com/WXYC/library-metadata-lookup/issues/1354) memory soak, which runs for up to 14 days and is invalidated by a restart or a worker-count change. The same change unset the two N=3-derived knobs step 3 of the unwind above points at — `DISCOGS_MAX_CONCURRENT` (was 1, now the stock 5) and `DISCOGS_BREAKER_REMAINING_FLOOR` (was 6, now the stock 3) — and raised `POSTHOG_RATELIMIT_EVENTS_PER_MINUTE` from 20 to 60 to hold the ~60/min per-environment ceiling at N=1. `DISCOGS_RATE_LIMIT` deliberately stays at 16 under the step-2 carve-out of the coupled-knob unwind above, because staging runs `DISCOGS_RATE_BUCKET_ENABLED=true` with its own `discogs-staging` bucket key. **If the Gate 0 comparison is ever re-run, it needs a fresh N=3 baseline window opened deliberately** — and not while a #1354 soak is in flight — rather than a restore of values this step recorded.
