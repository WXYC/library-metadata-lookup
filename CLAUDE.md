...
**Guard posture for a new `lml_cache` table** (LML#1204 item 9). Two fully-worked precedents enforce the org data-safety rule with opposite mechanics, and the choice between them is driven by what a lost row costs, not by taste. When the table holds **expensive-to-replace collected data** — rate-limited external API results whose loss means re-paying hours of collection (the offline streaming catalog) — use the `entity/streaming_catalog.py` posture: `BEFORE` triggers that `RAISE` on any transition discarding collected data, with a per-transaction GUC escape hatch for deliberate, scoped revocation; a bug anywhere in any writer must fail loudly at the database, because no caller can be trusted to re-derive the rule. When the table is a **cheap-to-refetch cache** — a wrongly-blocked write costs only staleness until the next fetch (the Wikipedia bio cache) — use the `entity/artist_wikipedia_bio.py` posture: an application-level `ON CONFLICT ... DO UPDATE ... WHERE` guard that silently refuses the destructive write, paired with a refusal clock (`last_refused_at` via `mark_artist_wikipedia_bio_refused`) so the refusal stays observable and drains deprioritize rather than re-hammer the row; here a hard database `RAISE` would be worse, since the guard fires in normal operation (repick divergence) and must degrade quietly on the request path. If unsure, ask which failure is recoverable by waiting: if re-fetching heals it, guard in the application; if only re-collection heals it, guard in the database.
