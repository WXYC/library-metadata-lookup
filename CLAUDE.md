...
- **[`docs/scripts.md`](docs/scripts.md)** — Streaming report stats regenerator, Discogs cache benchmark, API model generation, VA disambiguation, Bandcamp pipeline, resolver calibration, artist name variation audit, bulk artist-resolve drain (LML#759 PR D; drives prod `POST /api/v1/artists/resolve/bulk` over the BS#1614 name set)
- **[`docs/observability-rowless-flag.md`](docs/observability-rowless-flag.md)** — `LML_RESOLVE_NONLIBRARY_RELEASE` degradation alerts (LML#683): the PostHog + Sentry alerts on the LML#681 `cache.*` counters, their thresholds + 3-day baseline rationale, and the kill-switch response runbook; also the LML#879 `discogs_rate_gate_fail_open` unsampled counter for the shared Discogs rate bucket
