...
- **[`docs/api-endpoints.md`](docs/api-endpoints.md)** — Non-lookup endpoints: identity resolution (`/identity/resolve`, `/identity/bulk`, `/api/v1/identity/bulk-resolve-libraries`, `/api/v1/identity/resolve` — release-identity mint/lookup; validation in `identity/release_validation.py`, schema in `entity/release_identity.sql`), `/api/v1/artists/resolve/bulk` (bulk bare-name artist resolution, verify-before-mint; LML#759), `/api/v1/cache/refresh-for-identities` (source-agnostic cache warmer; LML#525), `/api/v1/streaming-check`, `/api/v1/releases/resolve`
