...
- **[`docs/deployment.md`](docs/deployment.md)** — Railway infrastructure (**Bucket-backed storage, no volume** — LML#834), branch strategy, CI/CD pipeline table, standalone `paths:`-filtered guard workflows (cross-cache-identity flag-doc consistency, plan-link citation checking — `scripts/check_plan_links.sh`, LML#1124), CI pin maintenance (Railway CLI, workflow `permissions:`, `@gha/v1` reusable refs), library.db / streaming_availability.db upload + download, the volume→bucket **cutover runbook** + **horizontal-scaling runbook**, `/health` semantics
- **[`docs/scripts.md`](docs/scripts.md)** — Streaming report stats regenerator, Discogs cache benchmark, API model generation, VA disambiguation, Bandcamp pipeline, resolver calibration, artist name variation audit, bulk artist-resolve drain (LML#759 PR D; drives prod `POST /api/v1/artists/resolve/bulk` over the BS#1614 name set), compilation track identity backfill (`scripts/backfill_compilation_track_identity.py`; LML#1020 — populates `lml_cache.compilation_track_identity` per CTA credit, Discogs leg over HTTP + cache-local MusicBrainz trigram leg, D5 recall-index position recovery), per-consumer API key seed/revoke/list (`scripts/api_keys/`; LML per-consumer API keys plan), the unmatched-compilation drain (`scripts/search_unmatched_compilations.py`; LML#1353 — where a guarded-matcher rejection may be relaxed to the title axis alone, and the `found_title_only` status that records it), the LML#513/#1192 Wikipedia URL extractor validation (`scripts/wikipedia_url_validation.py`) and bio-cache drain (`scripts/warm_wikipedia_bios.py`; incremental/`--retry-misses`/`--repick`/`--refresh-stale` modes, the primary population mechanism for `lml_cache.artist_wikipedia_bio`), streaming-links export (`scripts/export_streaming_links.py`; `mode=ro` over the precious artifact, invoked by discogs-etl's daily `sync-library.sh` and by this repo's `refresh-streaming.yml` — covers the LML#1352/#1353 Spotify match-provenance gate, its three callers and why the gated-row count is not a `streaming_links` delta, pointing at `_has_match_provenance`'s docstring for the census and the approaches deliberately NOT taken)
- **[`docs/observability-rowless-flag.md`](docs/observability-rowless-flag.md)** — `LML_RESOLVE_NONLIBRARY_RELEASE` degradation alerts (LML#683): the PostHog + Sentry alerts on the LML#681 `cache.*` counters, their thresholds + 3-day baseline rationale, and the kill-switch response runbook; also the LML#879 `discogs_rate_gate_fail_open` unsampled counter for the shared Discogs rate bucket, the LML#1049 `discogs_artist_breaker_shed` unsampled counter for artist-path saturation-breaker sheds, and the LML#513/#1192 `wikipedia_bio_*` counters (six seeded `cache.*` keys for the `LML_BIO_PREFER_WIKIPEDIA` cache/serve/warm split, plus two unsampled `wikipedia_bio_fetch_ok`/`_reject` counters for the background warm task's own fetch outcome)
...
- **[wxyc-shared](https://github.com/WXYC/wxyc-shared)** -- Shared API contract (`api.yaml`). Defines `LookupRequest`, `LookupResponse`, and related schemas. Python models are generated via `scripts/generate_api_models.sh` and committed to `generated/api_models.py`. The generated models are used as API boundary types; internal domain models (`LibraryItem`, `DiscogsSearchResult`) are converted via `to_catalog_item()` / `to_match_result()` methods. Generation runs with `--strict-nullable` ([WXYC/wxyc-shared#302](https://github.com/WXYC/wxyc-shared/issues/302)), which widens required-and-nullable fields to `X | None` and narrows optional-with-default fields off `Optional` -- see [`docs/scripts.md`](docs/scripts.md#strict-nullable-field-shape-semantics) for the LML-specific failure modes and override pattern. `generated/api_models.py` is a committed snapshot, not a live view of `api.yaml`: CI regenerates it against wxyc-shared `main` at run time and diffs, so a Codegen Freshness failure can mean the committed file has drifted behind upstream rather than that your change broke anything -- see [#1117](https://github.com/WXYC/library-metadata-lookup/issues/1117) for the currently-tracked drift.
...
Each lifespan bootstrap is **shape-verified** for the statements the parser recognizes (LML#1210): `entity/ddl.py` derives the tables, columns and indexes a call's statements claim to ensure and checks them against `pg_catalog`, so "schema ready" is a checked fact rather than an inference from statement success. Unrecognized forms — a callable, a `DO` block, a `DROP INDEX` — contribute no expectation and are verified not at all, deliberately: an expectation that under-reports what a statement does is the one direction that would let a genuinely-short table pass as healthy. A mismatch raises `LmlCacheSchemaMismatchError` and logs loudly, because the alternative symptom is `swallowing_fetch`/`swallowing_execute` quietly degrading every query to a miss. The error is caught per-label, so a mismatch degrades one cache and boot continues.
...
**Guard posture for a new `lml_cache` table** (LML#1204 item 9): expensive-to-recollect data (rate-limited API results) gets the database-level RAISE-trigger guard with a per-transaction GUC escape hatch, like `entity/streaming_catalog.py`; a cheap-to-refetch cache gets the application-side `ON CONFLICT ... WHERE` guard plus a refusal clock, like `entity/artist_wikipedia_bio.py`. Tiebreak: if re-fetching heals the loss, guard in the application; if only re-collection heals it, guard in the database. Full rationale lives in those two modules' docstrings.
