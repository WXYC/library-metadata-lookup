...
Every lifespan bootstrap is **shape-verified** after it runs (LML#1210): `entity/ddl.py` derives the tables, columns and indexes a call's statements claim to ensure and checks them against `pg_catalog`, so "schema ready" is a checked fact rather than an inference from statement success. A genuine mismatch raises `LmlCacheSchemaMismatchError` and logs loudly, because the alternative symptom is `swallowing_fetch`/`swallowing_execute` quietly degrading every query to a miss.
